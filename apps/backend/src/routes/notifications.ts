...
export async function registerNotificationsRoutes(fastify: FastifyInstance) {
  await fastify.register(async (fastify) => {
    ...
    const configGuard = { preHandler: authorize(CAPABILITIES.configNotifications) };
    const testGuard = { preHandler: authorize(CAPABILITIES.testNotifications) };

    fastify.get('/api/config/notifications', async (_request, reply) => {
    ...
    fastify.put('/api/config/notifications', configGuard, async (request, reply) => {
    ...
    fastify.get('/api/notifications/log', async (request, reply) => {
      ...
      return reply.send({ success: true, data: { rows, total, last24h } });
    });

    fastify.delete('/api/notifications/log/:id', configGuard, async (request, reply) => {
    ...
    fastify.post('/api/notifications/log/delete', configGuard, async (request, reply) => {
      const parsed = NotificationLogDeleteSchema.safeParse(request.body);
      if (!parsed.success) {
        return reply
          .status(400)
          .send({ success: false, error: 'Invalid delete request', issues: parsed.error.issues });
      }
      const removed = notificationLogDb.deleteByIds(parsed.data.ids);
      return reply.send({ success: true, data: { removed } });
    });

    fastify.post('/api/notifications/test', testGuard, async (request, reply) => {
      const parsed = NotificationTestRequestSchema.safeParse(request.body);
      if (!parsed.success) {
        return reply
          .status(400)
          .send({ success: false, error: 'Invalid test request', issues: parsed.error.issues });
      }
      ...
      return reply.send({ success: true, data: { results: outcome.results, allOk: outcome.ok } });
    });
  });
}
