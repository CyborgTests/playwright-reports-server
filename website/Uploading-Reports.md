...
`environment` is the conventional field for the deployment target (`qa`, `staging`, `prod`, …). Pass it in `resultDetails` (or as `--meta environment=qa` on a CLI upload) and the reports list shows it in a dedicated **Env** column; filter with `?environment=qa` on Reports, Results, Analytics, and Failure Clusters. Use `?environment=unknown` for runs uploaded without a normalized environment. Legacy tag filtering via `?tags=environment:qa` still works. Reports uploaded without it show **Unknown** in the Env column.
